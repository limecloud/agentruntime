...
| `start_benchmark_trial` | 把 dataset、task、configuration、Harbor job/trial、sandbox 和 timeout 绑定到 runtime run。 |
| `record_benchmark_reward` | 把 reward、reward details、verifier status、failure category 和 criterion summary 绑定到 trial。 |
| `export_benchmark_trial` | 导出 trajectory、runtime transcript、artifacts、reward refs、artifact manifest 和 Agent QC refs。 |
| `compare_benchmark_runs` | 记录 baseline/candidate deltas、cost、evidence completeness 与 promotion/revert decision。 |
