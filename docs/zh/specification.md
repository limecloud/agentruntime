...
10. Subagent graph、background jobs、large output storage 与 session reconstruction。
...
7. Jobs 需要 item 级状态、attempt、assignment 和 progress。
8. Remote channels 需要 channel identity、resume cursor、permission bridge 和断线语义。
9. Model routing 需要 task profile、candidate set、routing decision、fallback、single-candidate 和 no-candidate 事实。
10. Cost、quota、rate limit、request telemetry 和 evidence 必须能用同一组 correlation ids join。
