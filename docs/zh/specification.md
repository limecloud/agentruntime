...
## Profiles 与 conformance

Agent Runtime 按层标准化：

| Layer | 稳定性 | 含义 |
| --- | --- | --- |
| Public core | 可移植 | 跨产品适用的 identities、event envelope、control-plane 语义、snapshots 与 ownership rules。 |
| Recommended extensions | 可移植但可选 | Rich agent runtime 常用的 permission、sandbox、hooks、process、routing、task graph、subagent、job、channel、history 与 evidence 事件族。 |
| Product profile | 实现特定 | 由产品或宿主选择的更严格子集和 payload shape。Profile 可以要求 public core 之外的 ids、fields、fixtures 与 validation。 |

Profile 不能重新定义 ownership。它只能收紧公开模型、增加 typed payload fields，或要求特定 event families。[Lime AgentRuntime Profile](/zh/profiles/lime) 是 Lime current runtime 的参考 product profile。
...
11. Benchmark trial instrumentation：dataset/config/task correlation、trajectories、rewards 和 comparison decisions。
...
| Benchmark | `benchmark.dataset.resolved` / `benchmark.configuration.resolved` / `benchmark.trial.started` / `benchmark.trial.completed` / `benchmark.trial.failed` / `benchmark.reward.recorded` / `benchmark.comparison.completed` | 记录 hill-climbing trial、trajectory、reward 和 baseline/candidate decision。 |
...
| `start_benchmark_trial` / `record_benchmark_reward` / `export_benchmark_trial` / `compare_benchmark_runs` | Dataset、task、config、trial、reward 和 comparison scope。 | Benchmark trial events、reward refs、trajectories 和 promotion/revert decisions。 |
...
- `benchmark_summary`：dataset/task/config ids、trial status、reward refs、trajectory refs、aggregate delta 和 promotion/revert decision。
...
- Benchmark trials 导出 dataset/config/task ids、trajectories、reward details、runtime correlation 和 comparison decisions。
